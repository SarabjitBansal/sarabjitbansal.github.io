$(document).ready(function(){
  ...
  var instance = new TypeIt('#helloId', {
    strings: ["Namaste!","I'm Sarabjit Bansal.","Foodie & Full Stack Web Developer."],
    speed: 50,
    autoStart: false
  });
...
function Carousel(element) {
  ...
  this._container1 = element.querySelector('.container1');
  this._interval = null;
...
Carousel.prototype.handleEvent = function (event) {
  ...
  switch (event.type) {
    ...
    case 'touchstart':
      ...
      break;
    case 'touchmove':
      this._container1.style.transform = 'translate3d(' + (-position * 100) + 'vw, 0, 0)';
      break;
    case 'touchend':
      this._container1.style.transition = '';
    ...
    case 'transitionend':
      ...
      this._container1.style.transition = 'none';
      this._container1.style.transform = 'translate3d(' + (i < 0 ? -count * 100 : 100) + 'vw, 0, 0)';
      // Force changes to be applied sequentially by reflowing the element.
      this._container1.offsetHeight;
      this._container1.style.transition = '';
      this._container1.offsetHeight;
...
Carousel.prototype.set = function (i) {
  ...
  this._slide = i;
  this._container1.style.transform = 'translate3d(' + (-i * 100) + '%, 0, 0)';
  this._updateNav();
};

Carousel.prototype._countSlides = function () {
  return this._container1.querySelectorAll('.slide').length;
};
...
Carousel.prototype._updateNav = function () {
  ...
  for (var i = 0; i < count; i++) {
    if (i > 0) html += '&nbsp;';
    html += '<a' +  (i == this._slide ? ' class="current navClass"' : '') + ' class="navClass"'+ ' data-slide="' + i + '" href="#">●</a>';
  }
